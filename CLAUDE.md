...
Telegram bot for managing a shared Google Calendar via natural language (Korean). Users send messages in Telegram, OpenAI GPT-4.1 parses intent via function calling, and the bot executes corresponding Google Calendar API operations. Also supports Naver Maps navigation/directions.
...
7. For navigation: geocode via Naver API → prompt user for location share → build directions URL
...
- **naver_service.py** — Naver Maps geocoding API + directions URL builder.
...
- **Navigation two-step flow**: `navigate`/`navigate_to_event` geocodes destination and stores in `_pending_navigation`, then `handle_location` builds the Naver Maps directions URL.
...
Copy `.env.example` to `.env`. Required vars: `TELEGRAM_BOT_TOKEN`, `OPENAI_API_KEY`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `SHARED_CALENDAR_ID`. Optional: `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET` (for navigation), `GOOGLE_REDIRECT_URI`, `DAILY_REPORT_TIME`, `TIMEZONE`, `OAUTH_SERVER_PORT`.
