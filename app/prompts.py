...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "navigate",
            "description": "직접 장소명이나 주소로 길찾기를 제공합니다. '강남역 가는 길', '서울역 길찾기' 등 구체적 장소를 직접 지정할 때만 호출하세요. 캘린더 일정의 장소로 가려면 navigate_to_event를 사용하세요.",
            "parameters": {
                "type": "object",
                "properties": {
                    "destination": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "캘린더 일정의 장소(location)로 길찾기를 제공합니다. 사용자가 특정 일정의 장소로 가는 길을 요청할 때 호출하세요. 예: '4번 장소 가는법', '다음 일정 장소로 가는 길', '3월 20일 교육 장소 길찾기'",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
...
- 이전 대화에서 조회한 일정과 관련된 길찾기 요청("N번 장소 가려면", "N번 가는법", "그 일정 장소 길찾기", "거기 어떻게 가")은 반드시 navigate_to_event를 호출하세요. navigate가 아닌 navigate_to_event입니다. 이전 조회 결과에서 해당 일정의 제목과 날짜를 추출하여 title과 date 파라미터로 전달하세요.
