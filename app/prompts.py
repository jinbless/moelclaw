...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "navigate",
            "description": "직접 장소명이나 주소로 길찾기를 제공합니다. '강남역 가는 길', '서울역 길찾기' 등 구체적 장소를 직접 지정할 때만 호출하세요. ⚠️ '4번 일정', 'N번 일정', '그 일정', '위 일정'처럼 이전 대화의 일정 목록을 지칭하는 경우에는 절대 사용하지 말고 navigate_to_event를 사용하세요.",
            "parameters": {
                "type": "object",
                "properties": {
                    "destination": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "캘린더 일정의 장소(location)로 길찾기를 제공합니다. 사용자가 특정 일정의 장소로 가는 길을 요청할 때 호출하세요. 예: '4번 일정 길찾기', '4번 가는 방법', '3번 일정 어떻게 가', '다음 일정 장소로 가는 길', '3월 20일 교육 장소 길찾기'. ⚠️ 이전 대화에서 조회한 일정 목록의 번호(N번)를 지칭하는 길찾기는 반드시 이 함수를 사용하세요.",
            "parameters": {
                "type": "object",
                "properties": {
                    "title": {
                        "type": "string",
                        "description": "일정 제목 또는 키워드 (선택). 미입력 시 가장 가까운 다음 일정 사용",
                    },
                    "date": {
                        "type": "string",
                        "description": "일정 날짜 (YYYY-MM-DD 형식, 선택). 미입력 시 오늘 날짜 사용",
                    },
                },
                "required": [],
                "additionalProperties": False,
...
- 사용자가 구체적 장소명/주소로 길찾기를 요청하면 navigate 함수를 호출하세요. (예: "강남역 가는법", "서울역 길찾기")
- 이전 대화에서 조회한 일정과 관련된 길찾기 요청("N번 일정 길찾기", "N번 가는 방법", "N번 장소 가려면", "N번 가는법", "그 일정 장소 길찾기", "거기 어떻게 가")은 반드시 navigate_to_event를 호출하세요. navigate가 아닌 navigate_to_event입니다. 이전 조회 결과에서 해당 일정의 제목과 날짜를 추출하여 title과 date 파라미터로 전달하세요.
